...
{	
	return TransparentBlt(dcDst, x, y, psInfo->nWidth, psInfo->nHeight, psInfo->dcSurface, 0,0, psInfo->nWidth, psInfo->nHeight,
						  psInfo->crColorKey);
}

BOOL __gePutSprite(HDC dcDst, int x, int y, SURFACEINFO* psInfo, COLORREF color)
{
	return TransparentBlt(dcDst, x, y, psInfo->nWidth, psInfo->nHeight, psInfo->dcSurface, 0, 0, psInfo->nWidth, psInfo->nHeight,
		color);
}

//// hdc에 표면을 복사 (후면표면DC --전송--> 화면DC)
